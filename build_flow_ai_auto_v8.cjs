...
const contentJs = `// Flow Ai Auto v8.0 - Professional Studio with Multi-Image Slot (Product + Model + Location) & Anti-Duplicate Injection
(function() {
...
// Also update manifest.json name to "Flow Ai Auto"
const manifest = JSON.parse(fs.readFileSync(path.join(extDir, 'manifest.json'), 'utf8'));
manifest.name = "Flow Ai Auto";
manifest.description = "Ekstensi Resmi Flow Ai Auto - Otomasi Video Brutal Multi-Scene, Model Omni Flash & Veo 3.1 Suite dengan Injeksi Foto Produk, Model & Lokasi.";
fs.writeFileSync(path.join(extDir, 'manifest.json'), JSON.stringify(manifest, null, 2));

console.log("Flow Ai Auto v8.0 content.js and manifest.json built successfully!");
