...
const contentJs = `// Flow Ai Auto v11.0 - Touchmove Error Fixed, Full Nano Banana Suite, Auto Video Downloader & 2-Way Sync
(function() {
  ...
  // Embedded Pure SVG Icons (100% CSP Safe)
  const ICONS = {
  ...
  // ROBUST DRAGGING (Fixes: Ignored attempt to cancel a touchmove event with cancelable=false)
  function makeDraggable(element, handle) {
    ...
    function dragTouchStart(e) {
      ...
      pos3 = touch.clientX;
      pos4 = touch.clientY;
      document.addEventListener('touchend', closeDragTouch, { passive: true });
    ...
    function elementTouchDrag(e) {
      ...
      if (moveDist > 3) {
        if (e.cancelable) {
          e.preventDefault(); // SAFE: Only cancel when event is cancelable!
        }
...
console.log("content.js v11.0 with touchmove error fix built successfully!");
